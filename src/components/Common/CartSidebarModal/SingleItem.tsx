...
const SingleItem = ({ item, removeItemFromCart }: SingleItemProps) => {
  ...
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-blue-light-4/60 bg-white p-3 shadow-[0_8px_24px_-12px_rgba(147,51,234,0.08)] transition-all duration-200 hover:border-blue-light-3 hover:shadow-[0_12px_32px_-12px_rgba(147,51,234,0.14)]">
      <CartSidebarItemImage productImage={productImage} title={item.title} />
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          ...
          {item.price > item.discountedPrice && (
            <p className="text-custom-xs text-dark-4 line-through">
              ₹{item.price.toLocaleString("en-IN")}
            </p>
          )}
          {item.quantity > 1 && (
            <span className="rounded-full bg-blue-light-5 px-2 py-0.5 text-custom-xs font-semibold text-blue-dark">
              ×{item.quantity}
            </span>
          )}
        </div>
      ...
      <button
        ...
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-blue-light-4 bg-white text-dark-3 transition-all duration-200 hover:border-red-light-4 hover:bg-red-light-6 hover:text-red"
      >
        <svg className="fill-current" width="18" height="18" viewBox="0 0 22 22" fill="none">
          <path
            fillRule="evenodd"
            clipRule="evenodd"
...
function CartSidebarItemImage({
  ...
  return (
    <div className="relative flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-blue-light-4/50 bg-gradient-to-br from-blue-light-5 to-white">
      <div
