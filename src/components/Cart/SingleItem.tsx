...
const SingleItem = ({ item }: { item: CartItem }) => {
  ...
  return (
    <div className="flex items-center border-t border-blue-light-4/40 px-6 py-5 transition-colors hover:bg-blue-light-5/30 sm:px-8">
      <div className="min-w-[400px]">
        <div className="flex items-center gap-5">
          <div className="relative flex h-17.5 w-full max-w-[80px] items-center justify-center overflow-hidden rounded-xl border border-blue-light-4/50 bg-gradient-to-br from-blue-light-5 to-white">
            <div
            ...
            {productImage ? (
              ...
              <span className="relative z-1 text-custom-xs text-dark-4">No image</span>
            )}
          </div>
          <h3 className="line-clamp-2 font-semibold text-dark transition-colors hover:text-blue">
            {item.title}
          </h3>
        </div>
      </div>
      ...
      <div className="min-w-[180px]">
        <CartPagePrice item={item} />
      </div>

      <div className="min-w-[275px]">
        <div className="flex w-max items-center overflow-hidden rounded-full border border-blue-light-4 bg-white shadow-sm">
          <button
            type="button"
            onClick={handleDecreaseQuantity}
            aria-label="Decrease quantity"
            className="flex h-10 w-10 items-center justify-center text-dark-3 transition-colors hover:bg-blue-light-5 hover:text-blue"
          >
          ...
          <span className="flex h-10 w-12 items-center justify-center border-x border-blue-light-4 text-sm font-semibold text-dark">
            {quantity}
          </span>
          <button
            type="button"
          ...
          >
            <svg className="fill-current" width="18" height="18" viewBox="0 0 20 20" fill="none">
              ...
              />
              <path
                d="M9.99967 16.6667C9.53944 16.6667 9.16634 16.2936 9.16634 15.8334V4.16671C9.16634 3.70647 9.53944 3.33337 9.99967 3.33337C10.4599 3.33337 10.833 3.70647 10.833 4.16671V15.8334C10.833 16.2936 10.4599 16.6667 9.99967 16.6667Z"
                fill="currentColor"
              />
            </svg>
          </button>
      ...
      <div className="flex min-w-[50px] justify-end">
        <button
          ...
          className="flex h-9.5 w-full max-w-[38px] items-center justify-center rounded-xl border border-blue-light-4 bg-white text-dark-3 transition-all duration-200 hover:border-red-light-4 hover:bg-red-light-6 hover:text-red"
        >
...
function CartPagePrice({ item }: { item: CartItem }) {
  return (
    <div className="flex flex-col gap-0.5">
      <p className="font-bold text-blue-dark">₹{item.discountedPrice.toLocaleString("en-IN")}</p>
